 ...
 *
 * url: mydomain.com/index.php?eID=biblio_editorial
...
/** @var \Gabriel\Biblio\Domain\Repository\EditorialRepository $editorialRepository */
$editorialRepository = $objectManager->get('Gabriel\Biblio\Domain\Repository\EditorialRepository');

if ($datos->action == 'list')
{
    $editorials = $editorialRepository->findAll()->toArray();
    $total = count($editorials);
    $dataResponse = array();
    for( $i = 0; $i < $total; $i++) {
        /** @var \Gabriel\Biblio\Domain\Model\Editorial $editorial */
        $editorial = $editorials[$i];
        $dataResponse[$i] = $editorial->_getProperties();
    }
...
elseif ($datos->action == 'show')
{
    ...
    /** @var \Gabriel\Biblio\Domain\Model\Editorial $editorial */
    $editorial = $editorialRepository->findByUid($datos->uid);
    if($editorial == NULL){
        $eidUtil->JSendResponse('fail',$datos->uid.' no encontrado');
        die;
    }
    $eidUtil->JSendResponse('success',$editorial->_getProperties());
    die;
...
elseif ($datos->action == 'create')
{
    ...
    /** @var \Gabriel\Biblio\Domain\Model\Editorial $editorial */
    $editorial = $objectManager->get('Gabriel\Biblio\Domain\Model\Editorial');
    $editorial->setProperties(
        $datos->nombre
    );

    $editorialRepository->add($editorial);
    $persistenceManagerInterface->persistAll();
    $eidUtil->JSendResponse('success', $editorial->getUid() );
...
elseif ($datos->action == 'remove')
{
    ...
    $editorialRepository->remove($editorial);
    $persistenceManagerInterface->persistAll();
