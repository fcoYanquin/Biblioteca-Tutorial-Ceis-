 ...
 *
 * url: mydomain.com/index.php?eID=biblio_libro
...
elseif ($datos->action == 'show')
{
    ...
    /** @var \Gabriel\Biblio\Domain\Model\Libro $libro */
    $libro = $libroRepository->findByUid($datos->uid);
    if($libro == NULL){
        $eidUtil->JSendResponse('fail',$datos->uid.' no encontrado');
        die;
    }
    $eidUtil->JSendResponse('success',$libro->_getProperties());
    die;

}
elseif ($datos->action == 'prestamo')
{
    if( !isset($datos->nombre) ) {
        $eidUtil->JSendResponse('fail','no hay nombre');
        die;
    }

    /** @var \Gabriel\Biblio\Domain\Model\Libro $libro */
    $libro = $libroRepository->findByNombre($datos->nombre)->toArray();
    error_log(count($libro));

    if($libro == NULL){
        $eidUtil->JSendResponse('fail',$datos->nombre.' no encontrado');
        die;
    }
    $dataResponse = array();
    $total = count($libro);
    for ($i = 0; $i < $total; $i++){
        /** @var \Gabriel\Biblio\Domain\Model\Libro $lb */

        $lb = $libro[$i];
        $dataResponse[] = $lb->_getProperties();
    }
...
elseif ($datos->action == 'create')
{
    ...
    if(isset($datos->editorial) ) {
        /** @var \Gabriel\Biblio\Domain\Repository\EditorialRepository $libroRepository */
        $editorialRepository = $objectManager->get('Gabriel\Biblio\Domain\Repository\EditorialRepository');
        /** @var \Gabriel\Biblio\Domain\Model\Editorial $editorial */
        $editorial = $editorialRepository->findByUid($datos->editorial);
    }

    /** @var \Gabriel\Biblio\Domain\Model\Libro $libro */
    $libro = $objectManager->get('Gabriel\Biblio\Domain\Model\Libro');
    $libro->setProperties(
    ...
    $eidUtil->JSendResponse('success', $libro->getUid() );
