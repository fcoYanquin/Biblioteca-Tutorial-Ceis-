...
class AutorController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
     ...
     * @return void
     */
    public function showAction(\Gabriel\Biblio\Domain\Model\Autor $autor)
    {
     ...
     * action create
     *
     * @param \Gabriel\Biblio\Domain\Model\Autor $newAutor
     * @return void
     */
    public function createAction(\Gabriel\Biblio\Domain\Model\Autor $newAutor)
    {
     ...
     *
     * @param \Gabriel\Biblio\Domain\Model\Autor $autor
     * @ignorevalidation $autor
     * @return void
     */
    public function editAction(\Gabriel\Biblio\Domain\Model\Autor $autor)
    {
        $this->view->assign('autor', $autor);
        $tmp = 1;
    }
     ...
     * @return void
     */
    public function updateAction(\Gabriel\Biblio\Domain\Model\Autor $autor)
    {
     ...
     * @return void
     */
    public function deleteAction(\Gabriel\Biblio\Domain\Model\Autor $autor)
    {
    ...
    public function searchAction()
    {
        ...
        $autors = $this->autorRepository->findByLike($searchTerm);
        $this->view->setTemplatePathAndFilename(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('biblio') . 'Resources/Private/Templates/Autor/List.html');
        $this->view->assign('autors', $autors);
    ...
    public function generateExcelAction()
    {
        ...
        foreach ($autors as $autor) {
            $excel->getActiveSheet()->SetCellValue(\PHPExcel_Cell::stringFromColumnIndex(0) . $excelRowCounter, empty($autor->getNombre()) ? $noInfo : $autor->getNombre());
            $excelRowCounter++;
        }
    ...
    public function generateCSVAction()
    {
        ...
        foreach ($autors as $autor) {
            fputcsv($output, [
                    empty($autor->getNombre()) ? $noInfo : $autor->getNombre()
                ]);
        }
        die;
    }
    ...
    private function guardarCSVAction($archivoCSV)
    {
                    ...
                    if ($number_of_fields == $count) {
                        if ($current_row == 1) {
                            ...
                            if (!$campoVacio && !$campoInvalido) {
                                $nuevoRegistro = new \Gabriel\Biblio\Domain\Model\Autor();
        ...
        if (!$resultado->error) {
            foreach ($arrayRegistros as $registro) {
                $this->autorRepository->add($registro);
            }
        }
        return $resultado;
    }
